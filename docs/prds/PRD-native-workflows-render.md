...
goals: |
  ...
  context-store publish/discover) that later features build on.
---

# PRD: koto sessions render natively in Claude Code's `/workflows` (initial single-session slice)

## Status

Done

Requirements for the initial single-session slice of koto's native Claude Code
...
This is the initial single-session slice: the thinnest end-to-end slice that
proves the whole path (host publishes a location, koto writes its state there as
it advances, `/workflows` renders it) for one session, before the richer slices
are built on top.
...
- The slice lands the shared foundation the later features extend without
  ...
  key schema already supports a future hierarchy's ancestor walk.
...
- As a koto maintainer, I want the file contract, hook point, and context-store
  key chosen for extension, so that later features add to the foundation instead
  of reworking it.
...
- **R4 -- Extensible file contract.** The file is a minimal *valid* shape that
  later slices add fields to without breaking the initial readers. A
  koto-namespaced block identifies the file as koto's and carries a contract version.
- **R5 -- Publish/discover via the context store.** The target `/workflows`
  ...
  location (for a single session, that resolves to the session's own published
  location). The key schema is chosen so a future hierarchy's
  nearest-published-ancestor walk needs no change to it.
...
- **R11 -- Atomic, non-torn write.** The file is written so a concurrent
  ...
  correctness floor; the broader hardening guard is a later slice.)
...
- [ ] **AC1 (verified-when #1).** Driving a real koto session inside a real
...
- [ ] **AC2 (verified-when #2).** After the session advances and the
...
- [ ] **AC3 (verified-when #3).** On completion the entry reads *done*, not
  a stuck *running*. (Scripted: driving the session to a terminal state leaves
  the file at a done/failed status.)
- [ ] **AC4 (verified-when #4).** With no published location for the
...
- [ ] **AC6.** The published location lives in the context store under the
  ...
  schema is documented as supporting a future hierarchy's walk with no change.
...
- The version/fixture guard over the three undocumented surfaces and the
  rendered smoke check. This slice ships atomic write as a correctness
  floor but not the guard.
- Retention/rotation and crash-staleness reconciliation.
- Any koto skill, reader, MCP server, or parallel surface (settled by the
  surface decision).
...
- **Minimal projection now, richness later.** The BRIEF's scope fixes the
  initial slice at name + current state + running/done. Per-phase detail is
  deliberately deferred to the phase-detail enrichment so the initial slice
  proves the path cheaply; the file contract is extensible so that enrichment
  adds fields rather than reshaping.
- **Atomic write lands here, the guard lands later.** Atomic write is cheap
...
- **The context-store key schema is chosen up front for hierarchies.** Even
  though this slice has a single session, the key is namespaced and discovery is
  an ancestor walk, so a future hierarchy adds it without changing the shipped
  key (R5) -- the explicit "don't box the hierarchy slice out" obligation.
