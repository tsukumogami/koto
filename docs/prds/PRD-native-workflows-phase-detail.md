...
problem: |
  The initial render shows a koto session in Claude Code's `/workflows` as a bare
...
goals: |
  Enrich the single-session projection the initial render emits from a status
  into the session's real structure: ordered phases with the active one marked,
  the active phase's directive/label, per-phase evidence and gate outcomes, and a
  ...
  to the initial file contract additively (new fields, bumped version, the
  existing render preserved), with the contract's shape fixture extended so the
  future drift-guard can validate the enriched shape.
---
...
Requirements for the phase-detail enrichment, derived from the Accepted
`BRIEF-native-workflows-phase-detail`. The surface decision is settled, and the
initial render's foundation (the
...
The initial single-session slice proved the path and deliberately carried the
...
The gap is purely the projection: the initial render emits a status where the
screen can render structure koto already derives.

This feature enriches the single-session projection to close that gap. It is
one of four independent increments over the initial slice; it does not add
...
- The enriched file extends the initial contract additively: the initial shape
  and its render are preserved, new fields are namespaced/versioned, and the
  contract version is bumped so the future drift-guard has a stable anchor.
- The initial render's four "Verified when" checks continue to pass (no regression).
...
- As a koto maintainer, I want the enriched fields added to the initial contract
  additively with a bumped version and an extended shape fixture, so that the
  future drift-guard validates the new shape and the initial readers do not break.
...
- **R6 -- Additive contract extension.** The enriched fields are added to the
  initial `koto-<uuid>.json` contract without breaking its shape or its render:
  the initial top-level fields (`id`, `name`, `status`, `startTime`) and `koto`
  block are preserved, the new fields are namespaced/additive, and
  `contractVersion` is bumped.
- **R7 -- Shape-change / drift-guard obligation.** Because this change changes
  the file shape, the contract's shape fixture / contract check is extended (or,
  if the initial slice shipped none, the extended shape is documented in the
  contract) so the future drift-guard can validate the enriched shape per the
  stated soft coupling.

### Non-functional

- **R8 -- Preserve the initial foundation unchanged.** The commit-funnel
  ...
  published-presence are unchanged. This change adds derivation and fields, not
  new seams.
- **R9 -- No regression.** The initial render's four "Verified when" checks
...
- **R10 -- Best-effort, non-breaking derivation.** The enriched derivation is
  best-effort like the initial render's: any per-field derivation failure degrades gracefully
  (the field is omitted or empty) and never fails the commit or produces an
  invalid file.

## Acceptance Criteria

- [ ] **AC1 (verified-when #1).** Opening `/workflows` on a multi-phase koto
...
- [ ] **AC2 (verified-when #2).** The active phase's directive/label is
  legible. (Scripted: the emitted file carries the current state's directive
  text and a human-readable active-phase label.)
- [ ] **AC3 (verified-when #3).** A gate-blocked session renders as blocked,
  not running and not done. (Scripted: a session whose latest current-epoch gate
  did not pass emits `status: blocked`.)
- [ ] **AC4 (no regression).** The initial render's four checks still pass: the
  initial verification harness (or its enriched successor) passes, covering
  single-session render, update on reopen, done-on-completion, and
  default-path-untouched.
- [ ] **AC5.** The enriched file preserves the initial shape and render: the
  initial top-level fields and `koto` block are present and unchanged in meaning;
  the new fields are additive; `contractVersion` is bumped.
- [ ] **AC6.** The contract's shape fixture / documented contract is extended to
  cover the enriched shape so the future drift-guard can validate it.
- [ ] **AC7.** `cargo build`, `cargo test`, `cargo clippy -- -D warnings`, and
  ...
  and updated if this change changes any surface they document.

## Out of Scope

- Hierarchies: coordinator and delegates each rendering as their own entries.
  This change is one non-hierarchical session.
- The version/fixture guard and rendered smoke check over the undocumented
  surface. This change extends the fixture and documents the shape so the future
  guard can be built; it does not build the guard.
- Retention/rotation and crash-staleness.
- Nested single-run rendering, MCP, and any koto skill, reader, or parallel
  surface (settled by the surface decision).
- Reopening the initial render's commit-funnel hook, context-store
  publish/discover, atomic write, mkdir, or opt-in mechanism.
- The finer terminal-inference edge cases the initial render carried forward as
  known limitations (unnamed-failure states beyond the failure heuristic, missing/
...
- **Reuse the detail read seam, do not build a new derivation.** The richer
  ...
  keeps koto's model the single source of truth and matches the initial render's
  driver. A bespoke re-read would invite drift against koto's terminal/blocked semantics.
- **Per-phase outcomes need the full log, blocked needs the current epoch.**
  ...
  full-log per-state bucketing. This change reuses the same event-payload
  derivations for both scopes rather than inventing new ones.
- **Additive contract, bumped version.** The initial slice defined a minimal
  valid shape and a `contractVersion`; this change adds fields and bumps the
  version rather than reshaping, so the initial readers and the future drift-guard
  both anchor on a stable, versioned contract.
- **The shape-change obligation is discharged here.** The shape-change /
  drift-guard obligation makes each shape-changing slice responsible for
  extending the future guard fixture whenever it changes the file shape. This
  change extends the fixture / documents the enriched shape so the future guard
  is not left under-covering.
