...
- Does `koto-stability-tests/` pin anything about the CLI flag/subcommand surface (as opposed to just the wire format), which would matter if a new command or flag is added? Not independently verified beyond a directory listing in this pass.
- Does the team prefer extending `koto status` with a `--details` flag (minimal surface, reuses existing read path) over a standalone `koto state-info`/`phase-info` command (more discoverable as a dedicated "get my instructions back" verb, at the cost of duplicating `status`'s read-and-derive logic)? This is a judgment call this lead surfaces but doesn't resolve.
- Should the `--to` directed-transition path's unconditional `details` inclusion (no visit-count check) be fixed for consistency, independent of the phase-info decision?
- Is there a case for ALSO fixing `koto next --full` to be genuinely side-effect-free when called with no `--with-data`/`--to` (e.g., short-circuit before gate evaluation/default-action execution when the caller's only intent is to re-read), rather than adding a wholly separate command? That would keep `next --full` as the single next-directive entry point while making it safe for the recovery use case — an alternative design worth weighing against extending `status`.

## Summary

`koto next --full` does NOT safely cover koto#90's escape-hatch ask: it only controls whether the `details` field is included in the response (`mod.rs:4010`, `if full || count <= 1`) — it does not change dispatch behavior at all, so a bare `koto next --full` still runs gate evaluation with real event appends, can re-execute a state's `default_action` shell command, can silently auto-advance the workflow past the phase the agent meant to re-read, and can trigger terminal session cleanup, all with zero mutation-suppression from the `--full` flag. No existing command is read-only *and* keyed by workflow name *and* returns `details`/`directive`: `koto status <name>` is read-only and name-keyed but returns neither field (`mod.rs:4903-4909`); `koto dashboard --once` is read-only but its 8-column contract carries neither field; the dashboard's interactive-only `read_detail`/`DetailData` carries `directive` but not `details`, and isn't scriptable. So `phase-info`'s functional gap is real. The best-fitting, lowest-new-surface answer is extending `koto status` with an opt-in details flag rather than minting a new command — it already has the read-only contract, the workflow-name-only input, and the derived `current_state`/compiled-template objects the details lookup needs; if a standalone command is preferred instead for discoverability, `koto state-info <name>` fits the CLI's "state" vocabulary and bare-verb convention better than `koto phase-info`, which would be the first CLI-facing use of the UI-only "phase" noun that `workflows_surface` deliberately keeps separate from the engine/CLI's "state" vocabulary.
