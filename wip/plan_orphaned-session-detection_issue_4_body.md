...
`handle_list` (`src/cli/session.rs`, currently at line ~504) is presently a two-line function: it calls `backend.list()` and serializes the resulting `Vec<SessionInfo>` directly via `serde_json::to_string_pretty`. Because `SessionInfo` derives `Serialize`, once `<<ISSUE:3>>` lands, the raw `template_source_status` field (path/exists/machine_id) will already appear in this output with no code change here -- but the design also calls for a shared wording-formatting helper "consulted by both" `handle_status` and `handle_list` (Implementation Approach, Phase 4). That helper, `format_stale_template_source_note`, is defined in <<ISSUE:1>> (not here -- see plan-review Category D correction below), so this issue's job on the list side is to call it and apply that same backend-aware note text to each stale row, not merely let the raw struct fall through unformatted.

**Correction from plan review (Category D, round 2):** an earlier version of this issue defined the shared wording-formatting helper itself. That left <<ISSUE:5>> (which depends only on <<ISSUE:1>>, not this issue) hedging between "reuse this issue's helper if it landed first" or "write an equivalent inline check otherwise" -- the same missing-dependency-edge risk the round-1 review already flagged for the `is_cloud()` accessor, just recurring one layer up for the wording text itself. The helper now lives in <<ISSUE:1>> alongside the accessor; this issue only consumes it.
...
- [ ] Both `handle_status` and `handle_list` call `format_stale_template_source_note` (from <<ISSUE:1>> -- **do not** define a new formatting helper here; it already exists and is also needed by <<ISSUE:5>>, which depends only on <<ISSUE:1>> and would otherwise have no guaranteed access to a helper defined in this issue) with `backend.is_cloud()` as the argument, so both surfaces get identical wording and cannot drift out of sync with each other.
