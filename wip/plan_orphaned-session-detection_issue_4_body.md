...
One grounding correction against the actual source: the design describes the gate as `Backend::is_cloud()`, "a static, zero-cost discriminant already in hand, `cloud.rs:559-561`." Reading that code directly shows `is_cloud()` is defined only on the concrete `CloudBackend` struct (`impl CloudBackend { pub fn is_cloud(&self) -> bool { true } }`), not on the `Backend` enum (`src/session/mod.rs`, `pub enum Backend { Local(LocalBackend), Cloud(CloudBackend) }`) that `handle_status`/`handle_list` actually hold. There is no `Backend::is_cloud()` today. This issue must add that accessor to the `Backend` enum itself (e.g. `pub fn is_cloud(&self) -> bool { matches!(self, Backend::Cloud(_)) }`, or a match delegating to the existing `CloudBackend::is_cloud()`) before the wording helper can gate on it -- the acceptance criteria below are written against this corrected reality rather than the design's shorthand.
...
- [ ] `Backend` (`src/session/mod.rs`) gains a `pub fn is_cloud(&self) -> bool` method (`Backend::Local(_) => false`, `Backend::Cloud(b) => b.is_cloud()` or equivalent), since no such method exists on the enum today -- only on the concrete `CloudBackend` struct (`src/session/cloud.rs:559-561`). This is the discriminant the wording helper below gates on.
