...
- [ ] Message wording branches on `Backend::is_cloud()` (added in <<ISSUE:1>>, so it's guaranteed to exist regardless of implementation order relative to <<ISSUE:4>>; reuse <<ISSUE:4>>'s shared formatting helper if it has already landed, or an equivalent inline call to `Backend::is_cloud()` if this issue lands first -- either way, do not add a second `is_cloud`-equivalent accessor): a stale `LocalBackend` session gets direct wording (e.g. asserting the directory no longer exists), a stale `CloudBackend` session gets softened wording acknowledging a cross-machine resume may be the explanation. The underlying `Path::exists()` computation itself does not differ by backend.

**Correction from plan review (Category D):** an earlier version of this AC left the accessor's origin ambiguous ("Issue 4's helper if landed first, or an equivalent local check" otherwise) with no dependency edge forcing either order -- since Issue 5 declared only `Blocked by <<ISSUE:1>>`, it could land before Issue 4 and be forced to invent its own divergent wording-gate logic. Moving `Backend::is_cloud()` itself into <<ISSUE:1>> (which both this issue and Issue 4 already depend on) resolves the ambiguity without adding a new dependency edge between 4 and 5.
