...
- [ ] Message wording calls `format_stale_template_source_note(backend.is_cloud())` -- both defined in <<ISSUE:1>>, which this issue already depends on -- rather than hand-rolling any inline wording logic of its own: a stale `LocalBackend` session gets direct wording, a stale `CloudBackend` session gets softened wording acknowledging a cross-machine resume may be the explanation. The underlying `Path::exists()` computation itself does not differ by backend.

**Correction from plan review (Category D, rounds 1 and 2):** round 1 found this AC left `Backend::is_cloud()`'s origin ambiguous ("Issue 4's helper if landed first, or an equivalent local check" otherwise) with no dependency edge forcing either order. Moving the accessor into <<ISSUE:1>> fixed that, but round 2 found the *wording helper itself* (`format_stale_template_source_note`) still had the identical problem one layer up -- this AC still said to "reuse Issue 4's helper if landed first, or write an equivalent inline check otherwise." Both the accessor and the wording helper now live in <<ISSUE:1>>, which this issue already depends on, closing the gap for both without adding a new dependency edge to <<ISSUE:4>>.
