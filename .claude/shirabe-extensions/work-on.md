...
runs (`validate.yml`, `validate-plugins.yml`, `eval-plugins.yml`, `validate-docs.yml`), plus two
repo checks it does not (marked). Paths no entry names get the default, the Rust suite, which
does not check `scripts/`, `install.sh`, `.github/` or `.cargo/audit.toml` (CI's `cargo audit`
needs a separate install). This file is `@`-imported on every `/work-on` run, so it stays short.
...
- `docs/**` -> both of:
  ...
  - `git diff --name-only --diff-filter=ACMR origin/main...HEAD -- docs/ | grep -vE "(^|/)(evals|tests)/fixtures/" | xargs -r shirabe validate --visibility=public`
