...
- `docs/**` -> all of:
  ...
  - `shirabe validate --visibility=public --lifecycle .` (draft posture on purpose: an in-flight `/execute` chain keeps its PLAN, which the ready posture rejects)
- `test/functional/**` -> `make -C test/functional test-functional` (not in PR CI: the Go feature suite)
- `benches/**`, `Cargo.toml`, `Cargo.lock` -> `cargo bench --no-run` (not in PR CI: bench targets are `test = false`)
