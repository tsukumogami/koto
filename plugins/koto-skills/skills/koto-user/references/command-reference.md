...
**Not from inside a command koto is running.** A tick exports `KOTO_TICK_SESSION` before it evaluates a gate or runs a `default_action`, and children inherit it, so a `koto next` started from one of those commands is refused with `nested_invocation` (exit 2) before anything runs. Left alone, a nested tick advances the session while the tick that spawned it keeps reporting the state it started with. The refusal covers the whole process tree, so a tick on a *different* session from inside a command is refused too; every other `koto` subcommand, `koto context` included, still works from a command. The marker has no liveness -- a detached command outlives the tick and keeps it -- so a refusal naming a session that `koto status` shows idle is cleared for one invocation with `KOTO_TICK_SESSION= koto next <name>`. See `error-handling.md#when-the-tick-named-in-the-message-is-already-gone`.
...
The path always ends with the session name as the last component. This is the same path that `{{SESSION_DIR}}` resolves to everywhere koto substitutes it (see Variable substitution below). Always exits 0 — the path is computed from the name, not read from disk.
